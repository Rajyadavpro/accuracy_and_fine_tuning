...
For each Superbill allocation that has a RawJson:
  ...
  3. Parse the RawJson (AI's initial extraction).
  4. Query all related DB tables to get user-corrected values.
  5. Overlay the DB values onto the RawJson structure with original bounding boxes -> ground truth.
  6. Dispatch JSON structures to Service Bus.
  7. Save the new highest allocation ID back to ClickHouse.
...
# ---------------------------------------------------------------------------
# Build helpers – wrap a DB value in {"value": "...", "bounding_box": ...}
# ---------------------------------------------------------------------------
...
def ocr_node_with_bbox(value: Any, original_field: Optional[Dict]) -> Optional[Dict[str, Any]]:
    """Wrap a value into an OCR node, preserving 'bounding_box' if present."""
    if value is None:
        return None
    node: Dict[str, Any] = {"value": value}
    if original_field and isinstance(original_field, dict) and "bounding_box" in original_field:
        node["bounding_box"] = original_field["bounding_box"]
    return node


def ocr_node_str_with_bbox(v: Any, original_field: Optional[Dict]) -> Optional[Dict[str, Any]]:
    s = fmt_str(v)
    return ocr_node_with_bbox(s, original_field) if s is not None else None


def ocr_node_money_with_bbox(v: Any, original_field: Optional[Dict]) -> Optional[Dict[str, Any]]:
    s = fmt_money(v)
    return ocr_node_with_bbox(s, original_field) if s is not None else None


def ocr_node_date_with_bbox(v: Any, original_field: Optional[Dict]) -> Optional[Dict[str, Any]]:
    s = fmt_date(v)
    return ocr_node_with_bbox(s, original_field) if s is not None else None


def ocr_node_int_with_bbox(v: Any, original_field: Optional[Dict]) -> Optional[Dict[str, Any]]:
    val = fmt_int(v)
    return ocr_node_with_bbox(val, original_field) if val is not None else None
...
def build_gt_patient(db_patient: Optional[Dict], original: Optional[Dict]) -> Optional[Dict]:
    ...
    gt: Dict[str, Any] = {}
    gt["Patient_FN"]             = ocr_node_str_with_bbox(db_patient.get("Patient_FN"), orig.get("Patient_FN"))
    gt["Patient_LN"]             = ocr_node_str_with_bbox(db_patient.get("Patient_LN"), orig.get("Patient_LN"))
    gt["Patient_MI"]             = ocr_node_str_with_bbox(db_patient.get("Patient_MI"), orig.get("Patient_MI"))
    gt["Patient_Id"]             = ocr_node_str_with_bbox(db_patient.get("Patient_Id"), orig.get("Patient_Id"))
    gt["Patient_Account_Number"] = ocr_node_str_with_bbox(db_patient.get("Patient_Account_Number"), orig.get("Patient_Account_Number"))
    gt["Patient_Control_Number"] = ocr_node_str_with_bbox(db_patient.get("Patient_Control_Number"), orig.get("Patient_Control_Number"))
    gt["Patient_Group"]          = ocr_node_str_with_bbox(db_patient.get("Patient_Group"), orig.get("Patient_Group"))
    gt["Patient_Addr1"]          = ocr_node_str_with_bbox(db_patient.get("Patient_Addr1"), orig.get("Patient_Addr1"))
    gt["Patient_Addr2"]          = ocr_node_str_with_bbox(db_patient.get("Patient_Addr2"), orig.get("Patient_Addr2"))
    gt["Patient_City"]           = ocr_node_str_with_bbox(db_patient.get("Patient_City"), orig.get("Patient_City"))
    gt["Patient_State"]          = ocr_node_str_with_bbox(db_patient.get("Patient_State"), orig.get("Patient_State"))
    gt["Patient_Zip"]            = ocr_node_str_with_bbox(db_patient.get("Patient_Zip"), orig.get("Patient_Zip"))
    gt["Patient_DOB"]            = ocr_node_date_with_bbox(db_patient.get("Patient_DOB"), orig.get("Patient_DOB"))
    gt["Patient_Gender"]         = ocr_node_str_with_bbox(db_patient.get("Patient_Gender"), orig.get("Patient_Gender"))
    gt["Patient_Relationship"]   = ocr_node_str_with_bbox(db_patient.get("Patient_Relationship"), orig.get("Patient_Relationship"))
    gt["Patient_Marital_Status"] = ocr_node_str_with_bbox(db_patient.get("Patient_Marital_Status"), orig.get("Patient_Marital_Status"))
    gt["Patient_Primary_Phone_No"] = ocr_node_str_with_bbox(db_patient.get("Patient_Primary_Phone_No"), orig.get("Patient_Primary_Phone_No"))
    gt["Patient_Home_Phone_No"]    = ocr_node_str_with_bbox(db_patient.get("Patient_Home_Phone_No"), orig.get("Patient_Home_Phone_No"))
    gt["Patient_Primary_Email"]    = ocr_node_str_with_bbox(db_patient.get("Patient_Primary_Email"), orig.get("Patient_Primary_Email"))
    gt["Insured_Name"]           = ocr_node_str_with_bbox(db_patient.get("Insured_Name"), orig.get("Insured_Name"))
    return gt
...
def build_gt_payer(db_payer: Optional[Dict], original: Optional[Dict]) -> Optional[Dict]:
    ...
    orig = original if isinstance(original, dict) else {}
    gt: Dict[str, Any] = {}
    gt["Payer_Name"]  = ocr_node_str_with_bbox(db_payer.get("Payer_Name"), orig.get("Payer_Name"))
    gt["Payer_Id"]    = ocr_node_str_with_bbox(db_payer.get("Payer_Id"), orig.get("Payer_Id"))
    gt["Payer_Addr1"] = ocr_node_str_with_bbox(db_payer.get("Payer_Addr1"), orig.get("Payer_Addr1"))
    gt["Payer_Addr2"] = ocr_node_str_with_bbox(db_payer.get("Payer_Addr2"), orig.get("Payer_Addr2"))
    gt["Payer_City"]  = ocr_node_str_with_bbox(db_payer.get("Payer_City"), orig.get("Payer_City"))
    gt["Payer_State"] = ocr_node_str_with_bbox(db_payer.get("Payer_State"), orig.get("Payer_State"))
    gt["Payer_Zip"]   = ocr_node_str_with_bbox(db_payer.get("Payer_Zip"), orig.get("Payer_Zip"))
    gt["Payer_Type"]  = ocr_node_str_with_bbox(db_payer.get("Payer_Type"), orig.get("Payer_Type"))
    return gt
...
def build_gt_provider(db_provider: Optional[Dict], original: Optional[Dict]) -> Optional[Dict]:
    ...
    orig = original if isinstance(original, dict) else {}
    gt: Dict[str, Any] = {}
    gt["Provider_Name"]     = ocr_node_str_with_bbox(db_provider.get("Provider_Name"), orig.get("Provider_Name"))
    gt["Provider_NPI"]      = ocr_node_str_with_bbox(db_provider.get("Provider_NPI"), orig.get("Provider_NPI"))
    gt["Provider_Addr1"]    = ocr_node_str_with_bbox(db_provider.get("Provider_Addr1"), orig.get("Provider_Addr1"))
    gt["Provider_Addr2"]    = ocr_node_str_with_bbox(db_provider.get("Provider_Addr2"), orig.get("Provider_Addr2"))
    gt["Provider_City"]     = ocr_node_str_with_bbox(db_provider.get("Provider_City"), orig.get("Provider_City"))
    gt["Provider_State"]    = ocr_node_str_with_bbox(db_provider.get("Provider_State"), orig.get("Provider_State"))
    gt["Provider_Zip"]      = ocr_node_str_with_bbox(db_provider.get("Provider_Zip"), orig.get("Provider_Zip"))
    gt["Provider_FedId"]    = ocr_node_str_with_bbox(db_provider.get("Provider_FedId"), orig.get("Provider_FedId"))
    gt["Provider_TaxId"]    = ocr_node_str_with_bbox(db_provider.get("Provider_TaxId"), orig.get("Provider_TaxId"))
    gt["Provider_Taxonomy"] = ocr_node_str_with_bbox(db_provider.get("Provider_Taxonomy"), orig.get("Provider_Taxonomy"))
    return gt
...
def build_gt_facility(db_provider: Optional[Dict], original: Optional[Dict]) -> Optional[Dict]:
    ...
    orig = original if isinstance(original, dict) else {}
    gt: Dict[str, Any] = {}
    gt["Facility_Name"]     = ocr_node_str_with_bbox(db_provider.get("Provider_Name"), orig.get("Facility_Name"))
    gt["Facility_NPI"]      = ocr_node_str_with_bbox(db_provider.get("Provider_NPI"), orig.get("Facility_NPI"))
    gt["Facility_Addr1"]    = ocr_node_str_with_bbox(db_provider.get("Provider_Addr1"), orig.get("Facility_Addr1"))
    gt["Facility_Addr2"]    = ocr_node_str_with_bbox(db_provider.get("Provider_Addr2"), orig.get("Facility_Addr2"))
    gt["Provider_City"]     = ocr_node_str_with_bbox(db_provider.get("Provider_City"), orig.get("Provider_City"))
    gt["Facility_State"]    = ocr_node_str_with_bbox(db_provider.get("Provider_State"), orig.get("Facility_State"))
    gt["Facility_Zip"]      = ocr_node_str_with_bbox(db_provider.get("Provider_Zip"), orig.get("Facility_Zip"))
    gt["Facility_FedId"]    = ocr_node_str_with_bbox(db_provider.get("Provider_FedId"), orig.get("Facility_FedId"))
    gt["Facility_TaxId"]    = ocr_node_str_with_bbox(db_provider.get("Provider_TaxId"), orig.get("Facility_TaxId"))
    gt["Facility_Taxonomy"] = ocr_node_str_with_bbox(db_provider.get("Provider_Taxonomy"), orig.get("Facility_Taxonomy"))
    return gt
...
def build_gt_diagnosis(db_dx_rows: List[Dict], original: Optional[Dict]) -> Optional[Dict]:
    ...
    orig = original if isinstance(original, dict) else {}
    gt: Dict[str, Any] = {}
    gt["Primary_DX"] = ocr_node_str_with_bbox(dx.get("Primary_DX"), orig.get("Primary_DX"))
    for i in range(1, 13):
        key = f"Secondary_DX{i}"
        gt[key] = ocr_node_str_with_bbox(dx.get(key), orig.get(key))
    return gt


def build_gt_service_line(db_sl: Dict, original: Optional[Dict]) -> Dict:
    orig = original if isinstance(original, dict) else {}
    gt: Dict[str, Any] = {}
    gt["Service_From_Date"]  = ocr_node_date_with_bbox(db_sl.get("Service_From_Date"), orig.get("Service_From_Date"))
    gt["Service_To_Date"]    = ocr_node_date_with_bbox(db_sl.get("Service_To_Date"), orig.get("Service_To_Date"))
    gt["Procedure_Code"]     = ocr_node_str_with_bbox(db_sl.get("Procedure_Code"), orig.get("Procedure_Code"))
    gt["Mod1"]               = ocr_node_str_with_bbox(db_sl.get("Mod1"), orig.get("Mod1"))
    gt["Mod2"]               = ocr_node_str_with_bbox(db_sl.get("Mod2"), orig.get("Mod2"))
    gt["Mod3"]               = ocr_node_str_with_bbox(db_sl.get("Mod3"), orig.get("Mod3"))
    gt["Mod4"]               = ocr_node_str_with_bbox(db_sl.get("Mod4"), orig.get("Mod4"))
    gt["Service_Billed_Amt"] = ocr_node_money_with_bbox(db_sl.get("Service_Billed_Amt"), orig.get("Service_Billed_Amt"))
    gt["D_U"]                = ocr_node_int_with_bbox(db_sl.get("D_U"), orig.get("D_U"))
    gt["Place_Of_Service"]   = ocr_node_str_with_bbox(db_sl.get("Place_Of_Service"), orig.get("Place_Of_Service"))
    gt["DX_1"]               = ocr_node_str_with_bbox(db_sl.get("DX_1"), orig.get("DX_1"))
    gt["DX_2"]               = ocr_node_str_with_bbox(db_sl.get("DX_2"), orig.get("DX_2"))
    gt["DX_3"]               = ocr_node_str_with_bbox(db_sl.get("DX_3"), orig.get("DX_3"))
    gt["DX_4"]               = ocr_node_str_with_bbox(db_sl.get("DX_4"), orig.get("DX_4"))
    gt["User_Status"]        = ocr_node_str_with_bbox(db_sl.get("User_Status"), orig.get("User_Status"))
    return gt
...
def build_gt_claim(
    ...
    """Build a single Superbill Claim ground-truth dict from DB rows."""
    gt: Dict[str, Any] = {}
    orig = original_claim if isinstance(original_claim, dict) else {}
    if original_claim:
        gt.update(original_claim)

    # --- Claim-level OCR value fields ---
    gt["Patient_Control_Number"] = ocr_node_str_with_bbox(db_claim.get("Patient_Control_Number"), orig.get("Patient_Control_Number"))
    gt["Claim_Total_Charge_Amt"] = ocr_node_money_with_bbox(db_claim.get("Claim_Total_Charge_Amt"), orig.get("Claim_Total_Charge_Amt"))
    gt["Claim_Filing_indicator"] = ocr_node_str_with_bbox(db_claim.get("Claim_Filing_indicator"), orig.get("Claim_Filing_indicator"))
    gt["Claim_Frequency_Code"]   = ocr_node_str_with_bbox(db_claim.get("Claim_Frequency_Code"), orig.get("Claim_Frequency_Code"))
    gt["Claim_Date_of_Service"]  = ocr_node_date_with_bbox(db_claim.get("Claim_Date_of_Service"), orig.get("Claim_Date_of_Service"))
    gt["Claim_Auth_No"]          = ocr_node_str_with_bbox(db_claim.get("Claim_Auth_No"), orig.get("Claim_Auth_No"))
    gt["Patient_Paid_Amt"]       = ocr_node_money_with_bbox(db_claim.get("Patient_Paid_Amt"), orig.get("Patient_Paid_Amt"))
    gt["CLIA_Number"]            = ocr_node_str_with_bbox(db_claim.get("CLIA_Number"), orig.get("CLIA_Number"))
    gt["CLaim_Invoice_Number"]   = ocr_node_str_with_bbox(db_claim.get("CLaim_Invoice_Number"), orig.get("CLaim_Invoice_Number"))
    gt["Admission_Date"]         = ocr_node_date_with_bbox(db_claim.get("Admission_Date"), orig.get("Admission_Date"))
    gt["Patient_Last_Seen_Date"] = ocr_node_date_with_bbox(db_claim.get("Patient_Last_Seen_Date"), orig.get("Patient_Last_Seen_Date"))
    ...
    # --- Sub-entities: Payer (List) ---
    original_payers = (orig.get("Payer") or [])
    gt_payers = []
    for idx, db_payer in enumerate(payer_rows):
        orig_p = original_payers[idx] if idx < len(original_payers) else None
        gt_payers.append(build_gt_payer(db_payer, orig_p))
    ...
    gt["BillingProvider"] = build_gt_provider(
        role_to_provider.get("BillingProvider"),
        orig.get("BillingProvider")
    )
    gt["ServicingProvider"] = build_gt_provider(
        role_to_provider.get("ServicingProvider"),
        orig.get("ServicingProvider")
    )
    gt["ReferringProvider"] = build_gt_provider(
        role_to_provider.get("ReferringProvider"),
        orig.get("ReferringProvider")
    )
    gt["OrderingProvider"] = build_gt_provider(
        role_to_provider.get("OrderingProvider"),
        orig.get("OrderingProvider")
    )
    gt["SupervisingProvider"] = build_gt_provider(
        role_to_provider.get("SupervisingProvider"),
        orig.get("SupervisingProvider")
    )
    gt["ServicingFacility"] = build_gt_facility(
        role_to_provider.get("ServiceFacilityProvider"),
        orig.get("ServicingFacility")
    )
    ...
    gt["ClaimDiagnosisCodes"] = build_gt_diagnosis(
        db_dx,
        orig.get("ClaimDiagnosisCodes"),
    )

    # --- Service Lines ---
    db_sls = service_lines_by_claim_id.get(claim_id, [])
    orig_sls = orig.get("ServiceLines") or []
    gt_sls = []
    for idx, db_sl in enumerate(db_sls):
        orig_sl = orig_sls[idx] if idx < len(orig_sls) else None
        gt_sls.append(build_gt_service_line(db_sl, orig_sl))
    gt["ServiceLines"] = gt_sls
...
def dated_json_filename(filename: str) -> str:
    print(f"dated_json_filename called with {filename}")
    stem, ext = os.path.splitext(filename)
    if not ext:
        ext = ".json"
        stem = filename
