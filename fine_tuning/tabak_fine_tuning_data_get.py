...
def _load_checkpoint_from_langfuse(checkpoint_dataset_name: str) -> Optional[str]:
    ...
    try:
        last_id = load_checkpoint_str(TABAK_FINETUNING_CHECKPOINT_TABLE, get_environment())
        if last_id is None:
...
def _save_checkpoint_to_langfuse(checkpoint_dataset_name: str, langfuse_environment: str, last_id: str) -> None:
    ...
    try:
        save_checkpoint_str(TABAK_FINETUNING_CHECKPOINT_TABLE, get_environment(), last_id)
...
def tabak_fine_tuning_data_push(
    ...
    checkpoint_dataset_name = f"tabak_finetuning_checkpoint_{clickhouse_environment}"
