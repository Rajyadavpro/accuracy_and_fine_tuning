...
# Azure Blob Config
EOB_CONTAINER = os.getenv("EOB_CONTAINER", "eob-json-accuracy-finetuning")
AZURE_CONN_STR = os.getenv("AZURE_STORAGE_CONNECTION_STRING")

def upload_eob_to_blob(base_filename: str, raw_json: str, ground_truth: dict) -> None:
    """
    Uploads raw JSON and ground truth JSON to Azure Blob Storage for EOBs.
    """
    ...
    try:
        ...
        container_client = blob_service_client.get_container_client(EOB_CONTAINER)
        ...
        # Upload Ground Truth (with default=str to handle Datetime/Decimal from SQL)
        ...
        _log(f"[Azure] Uploaded {raw_blob_name} and {gt_blob_name} to {EOB_CONTAINER}")
    ...
    with conn:
        ...
        while True:
            try:
                ...
                for row in rows:
                    row_id = row.get("Id")
                    file_name_val = row.get("File_name") or f"allocation_{row_id}"
                    raw_json_val = row.get("rawJson") or row.get("RawJson")

                    # Handle Blob upload for Raw JSON and Ground Truth
                    if raw_json_val:
                        # Ground truth dict excludes the raw JSON 
                        ground_truth_dict = {k: v for k, v in row.items() if k.lower() != "rawjson"}
                        upload_eob_to_blob(file_name_val, raw_json_val, ground_truth_dict)
