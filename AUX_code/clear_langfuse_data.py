...
# #     "idp_accuracy_checkpoint_exp","tabak_predictions_exp2"
# # ]
...
def clean_all_langfuse_datasets():
    ...
    # ---------------------------------------------------------
    # Step 1: Programmatically retrieve all dataset names
    # ---------------------------------------------------------
    logging.info("Retrieving all datasets in the project...")
    dataset_names = []
    page = 1
    
    while True:
        try:
            # Query the Langfuse API to retrieve project datasets with pagination
            ...
            logging.error(f"Error fetching dataset list on page {page}: {e}")
            break

    if not dataset_names:
        logging.info("No datasets found in this project to clean.")
        return

    logging.info(f"Found {len(dataset_names)} datasets to clean: {dataset_names}")

    # ---------------------------------------------------------
    # Step 2: Iterate and clean each retrieved dataset
    # ---------------------------------------------------------
    for dataset_name in dataset_names:
        logging.info(f"\n--- Starting cleanup for dataset: '{dataset_name}' ---")
        
        # A. Fetch and Delete all Experiment Runs (Paginated)
        logging.info(f"Fetching experiment runs for '{dataset_name}'...")
        run_names = []
        run_page = 1
        while True:
            try:
                runs_response = langfuse.api.datasets.get_runs(
                    dataset_name=dataset_name, 
                    page=run_page, 
                    limit=100
                )
                if not runs_response or not runs_response.data:
                    break
                ...
                logging.warning(f"Could not retrieve runs on page {run_page} for dataset '{dataset_name}': {e}. Stopping retrieval.")
                break
        ...
        # B. Fetch and Delete all Dataset Items (Paginated)
        logging.info(f"Fetching dataset items for '{dataset_name}'...")
        item_ids = []
        item_page = 1
        while True:
            try:
                ...
                logging.warning(f"Could not list items on page {item_page} for '{dataset_name}': {e}")
                break
        ...
        logging.info(f"Finished cleanup of all runs and items for dataset: '{dataset_name}'")
        logging.info("NOTE: The empty dataset container itself must be deleted manually via the Langfuse Web UI.\n")


if __name__ == "__main__":
    clean_all_langfuse_datasets()
