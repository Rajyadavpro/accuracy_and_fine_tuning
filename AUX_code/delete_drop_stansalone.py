...
1. Drops all tables in ClickHouse (completely removes them).
...
def clickhouse_request(host, http_port, user, password, query: str, is_post: bool = False, timeout: int = 30) -> requests.Response:
    """Helper to dispatch GET or POST requests to the ClickHouse HTTP interface."""
    url = f"http://{host}:{http_port}/"
    if is_post:
        return requests.post(url, auth=(user, password), timeout=timeout, data=query.encode('utf-8'))
    else:
...
def get_all_tables(host, http_port, database, user, password):
    """Retrieves a list of all tables inside the targeted ClickHouse database using system.tables."""
    try:
        query = f"SELECT name FROM system.tables WHERE database = '{database}' FORMAT TSV"
...
def drop_table(host, http_port, database, user, password, table_name) -> bool:
    """Drops (completely deletes) a specific table."""
    try:
        ddl = f"DROP TABLE IF EXISTS `{database}`.`{table_name}`"
        logger.info(f"[ClickHouse] Sending DROP query for '{table_name}'...")
        response = clickhouse_request(host, http_port, user, password, ddl, is_post=True)
        
        if response.status_code in [200, 201, 202]:
            logger.info(f"[ClickHouse] Table '{table_name}' dropped successfully!")
            return True
        else:
            logger.error(f"[ClickHouse] Drop failed on '{table_name}': {response.status_code} - {response.text[:500]}")
            return False
    except Exception as ex:
        logger.error(f"[ClickHouse] Exception dropping table '{table_name}': {ex}")
        return False


def run_clickhouse_cleanup(settings: dict) -> bool:
    """Coordinates the retrieval, display, deletion (drop), and verification of ClickHouse tables."""
    print("\n" + "="*80)
    print(" 1. CLICKHOUSE DATABASE TABLE DROPS ".center(80, "="))
    print("="*80)
    ...
    print(f"\n📊 Found {len(tables)} table(s) in ClickHouse Database '{database}':")
    for table in tables:
        print(f"  • {table}")
    ...
    successful_drops = 0
    for table in tables:
        if drop_table(host, http_port, database, user, password, table):
            successful_drops += 1
    ...
    remaining_tables = get_all_tables(host, http_port, database, user, password)
    
    if remaining_tables is not None and len(remaining_tables) == 0:
        print(f"  ✅ All tables dropped successfully from '{database}'.")
        return True
    elif remaining_tables:
        print(f"  ❌ Failed to clear all tables. {len(remaining_tables)} tables remaining:")
        for t in remaining_tables:
            print(f"     - {t}")
        return False
    else:
        print("  ❌ Verification failed due to an error connecting to ClickHouse.")
        return False
...
def main():
    ...
    # 3. Step 1: Run Clickhouse Table Drops
    ...
    print("="*60)
    print(f"  • ClickHouse Drop:     {'SUCCESS' if clickhouse_success else 'FAILED'}")
