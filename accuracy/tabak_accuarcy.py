...
# DEFAULT_MAX_MESSAGES_PER_RUN = None  # None = unlimited
...
#     query = f"""
#         SELECT Transation_id, file_receive_date
#         FROM `Transactions`
#         WHERE {' AND '.join(filters)}
#         ORDER BY file_receive_date ASC;
#     """
...
# def _checkpoint_table(folder_name: str) -> str:
#     return f"tabak_accuracy_checkpoint_{folder_name}"


# def _load_checkpoint(folder_name: str) -> Optional[datetime]:
#     """Load the last processed file_receive_date from the dedicated checkpoint table."""
#     try:
#         raw = load_checkpoint_str(_checkpoint_table(folder_name), ENVIRONMENT)
#         if raw:
...
# def _save_checkpoint(folder_name: str, file_receive_date: datetime) -> None:
#     """Persist the latest processed file_receive_date into the checkpoint table."""
#     try:
#         save_checkpoint_str(
#             _checkpoint_table(folder_name),
#             ENVIRONMENT,
#             file_receive_date.isoformat(),
#         )
#     except Exception as ex:
#         print(f"[ClickHouse] Error saving checkpoint: {ex}")


# def _send_batch_to_queue(ids: List[int]) -> None:
#     """Send a batch of Transation_ids to the Service Bus queue."""
...
#     print(f"[Queue] Sent batch of {len(ids)} IDs.")
...
# def main(
#     ids_per_message: int = DEFAULT_IDS_PER_MESSAGE,
#     max_messages_per_run: Optional[int] = DEFAULT_MAX_MESSAGES_PER_RUN,
...
#     if not start_datetime and args.period == "all":
#         checkpoint = _load_checkpoint(folder_name)
#         if checkpoint:
#             start_datetime = checkpoint
#             print(f"[Checkpoint] Resuming from: {start_datetime.isoformat()}")
#         else:
#             print("[Checkpoint] No prior checkpoint found. Fetching all data from beginning.")
...
#         query, query_params = build_query(args.period, start_datetime, end_datetime)
#         rows = run_query(conn, query, query_params)
#         print(f"✓ Connected. Period: {args.period}. Fetched {len(rows)} matching records.")

#         if not rows:
#             print("[Queue] No new records to send.")
#             return 0

#         # Dispatch in batches of ids_per_message; respect max_messages_per_run
#         messages_sent = 0
#         for batch_start in range(0, len(rows), ids_per_message):
#             if max_messages_per_run is not None and messages_sent >= max_messages_per_run:
#                 print(f"[Queue] Reached max_messages_per_run={max_messages_per_run}. Stopping.")
#                 break

#             batch = rows[batch_start : batch_start + ids_per_message]
#             id_batch = [int(r[0]) for r in batch]
#             _send_batch_to_queue(id_batch)

#             # Checkpoint: save the latest file_receive_date seen in this batch
#             latest_dt = batch[-1][1]
#             if hasattr(latest_dt, "isoformat"):
#                 _save_checkpoint(folder_name, latest_dt)

#             messages_sent += 1

#         print(f"[Queue] Done. Sent {messages_sent} message(s) covering {len(rows)} records.")
...
---------------------------------------
This script connects directly to a MariaDB database, extracts target audit fields 
utilizing database-level JSON capabilities, and queues them to Azure Service Bus.

It supports time-period filtering (--period), and infers incremental checkpoints 
from the latest saved Transation_id in ClickHouse, ensuring it always starts 
from at least ID 33049.
"""
...
def build_query(
    ...
    # Add the transaction ID to params
    params.append(str(last_transaction_id))
...
def _load_checkpoint_id(folder_name: str) -> Optional[int]:
    """Load the last processed Transation_id from the dedicated checkpoint table."""
    try:
        raw = load_checkpoint_str(_checkpoint_table(folder_name), ENVIRONMENT)
        if raw:
            return int(raw)
...
def _save_checkpoint_id(folder_name: str, last_id: int) -> None:
    """Persist the latest processed Transation_id into the checkpoint table."""
    try:
        save_checkpoint_str(
            _checkpoint_table(folder_name),
            ENVIRONMENT,
...
def main(
    ...
    # 1. LOAD CHECKPOINT ID AND CALCULATE MAX
    checkpoint_id = _load_checkpoint_id(folder_name)
    baseline_id = 33049
    
    if checkpoint_id:
        effective_last_id = max(baseline_id, checkpoint_id)
        print(f"[Checkpoint] Resuming from Transation_id > {effective_last_id} (Checkpoint was {checkpoint_id})")
    else:
        effective_last_id = baseline_id
        print(f"[Checkpoint] No prior checkpoint found. Using baseline Transation_id > {effective_last_id}")
    ...
    try:
        # 2. PASS effective_last_id TO build_query
        query, query_params = build_query(args.period, start_datetime, end_datetime, effective_last_id)
        rows = run_query(conn, query, query_params)
        print(f"✓ Connected. Period: {args.period}. Fetched {len(rows)} matching records.")
        ...
        for batch_start in range(0, len(rows), ids_per_message):
            ...
            # 3. SAVE THE LATEST Transation_id AS THE NEW CHECKPOINT
            latest_id = id_batch[-1]
            _save_checkpoint_id(folder_name, latest_id)

            messages_sent += 1

        print(f"[Queue] Done. Sent {messages_sent} message(s) covering {len(id_batch) + (messages_sent-1)*ids_per_message} records.")
