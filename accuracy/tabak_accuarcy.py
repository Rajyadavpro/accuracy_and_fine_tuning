...
def compute_metrics(results: List[Dict[str, str]]) -> Dict[str, Any]:
    total = len(results)
    correct_count = sum(
        1
        for item in results
        if is_prediction_correct(
            item["gen_cat"], item["gen_sub"], item["user_cat"], item["user_sub"]
        )
    )
...
def compute_daily_metrics(results: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    grouped_results: Dict[str, List[Dict[str, str]]] = {}
    for item in results:
        grouped_results.setdefault(item["source_date"], []).append(item)

    daily_rows: List[Dict[str, Any]] = []
    for source_date in sorted(grouped_results):
        daily_metrics = compute_metrics(grouped_results[source_date])
        daily_rows.append(
            {
                "source_date": source_date,
                "correct_predictions": daily_metrics["correct"],
                "incorrect_predictions": daily_metrics["incorrect"],
                "total_records": daily_metrics["total"],
                "accuracy_pct": daily_metrics["accuracy_pct"],
            }
        )
    return daily_rows


def compute_daily_breakdowns(
    ...
    for source_date in sorted(grouped_results):
        ...
        for row in daily_metrics["category_rows"]:
            category_rows.append(
                {
                    "source_date": source_date,
                    **row,
        ...
        for row in daily_metrics["subcategory_rows"]:
            subcategory_rows.append(
                {
                    "source_date": source_date,
                    **row,
    ...
    return category_rows, subcategory_rows
...
def _get_langfuse_client():
    try:
        ...
        print(f"[Langfuse] Error initializing client: {ex}")
        return None
...
def _upload_to_langfuse(
    ...
    daily_rows: List[Dict[str, Any]],
    daily_category_rows: List[Dict[str, Any]],
    daily_subcategory_rows: List[Dict[str, Any]],
    period: str,
    start_datetime: Optional[datetime],
    end_datetime: Optional[datetime],
...
def print_metrics(
    ...
    2. Category-Wise performance metrics (TP, FP, FN, Precision, Recall).
    3. Subcategory-Wise performance metrics (TP, FP, FN, Precision, Recall) under 'Others'.
    """
    total = metrics["total"]
    if total == 0:
    ...
    for row in daily_rows:
        ...
        print(f"AUDIT ACCURACY SUMMARY [{period_label.upper()}]")
        print("=" * 70)
        print(f"Total evaluated records : {row['total_records']}")
        print(f"Correct predictions      : {row['correct_predictions']}")
        print(f"Incorrect predictions    : {row['incorrect_predictions']}")
        print(f"Date                     : {_display_date(row['source_date'])}")
        print("-" * 70)
        print(f"OVERALL ACCURACY         : {row['accuracy_pct']:.2f}%")
        print("=" * 70)

    grouped_category_rows: Dict[str, List[Dict[str, Any]]] = {}
    for row in daily_category_rows:
        grouped_category_rows.setdefault(row["source_date"], []).append(row)

    for source_date in sorted(grouped_category_rows):
        print("\nCATEGORY PERFORMANCE")
        print("-" * 86)
        print(
            f"{'Category':<22} | {'TP':^5} | {'FP':^5} | {'FN':^5} | {'Prec':^7} | {'Rec':^7} | {'Date':<10}"
        )
        for row in grouped_category_rows[source_date]:
            print(
                f"{row['category']:<22} | {row['tp']:5d} | {row['fp']:5d} | {row['fn']:5d} | "
                f"{row['precision_pct']:6.1f}% | {row['recall_pct']:6.1f}% | {_display_date(source_date):<10}"
            )

    grouped_subcategory_rows: Dict[str, List[Dict[str, Any]]] = {}
    for row in daily_subcategory_rows:
        grouped_subcategory_rows.setdefault(row["source_date"], []).append(row)

    for source_date in sorted(grouped_subcategory_rows):
        print("\nSUBCATEGORY PERFORMANCE (Others)")
        print("-" * 96)
        print(
            f"{'Subcategory':<32} | {'TP':^5} | {'FP':^5} | {'FN':^5} | {'Prec':^7} | {'Rec':^7} | {'Date':<10}"
        )
        for row in grouped_subcategory_rows[source_date]:
            print(
...
def main():
    ...
    try:
        ...
        # Structure query results sequentially
        results = [{
            "source_date": r[0].date().isoformat() if hasattr(r[0], "date") else str(r[0])[:10],
        ...
        # Process and output final diagnostic tables
        metrics = compute_metrics(results)
        daily_rows = compute_daily_metrics(results)
        daily_category_rows, daily_subcategory_rows = compute_daily_breakdowns(results)
        print_metrics(metrics, args.period, daily_rows, daily_category_rows, daily_subcategory_rows)
